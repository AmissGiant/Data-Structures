...
public class Board {
    ...
    public void updateOpenSpaces() {
        ArrayList<BoardSpot> openSpacesTwo = new ArrayList<BoardSpot>();
        for (int i = 0; i < gameBoard.length; i++){
            for (int j = 0; j < gameBoard[i].length; j++){
                if ( gameBoard[i][j] == 0 ){
                    BoardSpot Object = new BoardSpot(i, j);
                    openSpacesTwo.add(Object);
                }
            }
            openSpaces = openSpacesTwo;
        }
    }
    ...
    public void addRandomTile() {
        ...
        double randomValue = StdRandom.uniform(0, 10);
        double track = randomValue;
        int tileValue = 0;
        if(track < 9){
            tileValue = 2;
        }else{
        ...
        for (int i = 0; i < gameBoard.length; i++) {
            ...
            for (int j = 0; j < gameBoard[i].length; j++) {
                if (gameBoard[i][j] != 0) {
                    ...
                    gameBoard[i][j] = 0;
                    gameBoard[i][insertIndex++] = currentValue;
                }
            }
        }
    }
    ...
    for (int i = 0; i < gameBoard.length; i++) {
        for (int j = 0; j < gameBoard[i].length; j++) {
            transposedBoard[j][i] = gameBoard[i][j];
        }
    }
    gameBoard = transposedBoard;
    }
    ...
    public void flipRows() {
        ...
        int[][] flippedBoard = new int[rows][cols];
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                flippedBoard[i][j] = gameBoard[rows - 1 - i][j];
            }
        }
        gameBoard = flippedBoard;
    }
